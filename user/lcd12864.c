 ...
 *
 * PE7  = CS   (P90)  chip select, active high
 * PE8  = STD  (P89)  serial data SID
 * PE9  = SCLK (P88)  serial clock
 * PE10 = BLA  (P85)  backlight, high=on
 * PE11 = RST  (P84)  reset, active low
 *
...
void LCD12864_Init(void)
{
    ...
    /* PE7-PE11 as push-pull outputs */
    GPIO_InitStructure.GPIO_Pin   = LCD_CS_PIN | LCD_STD_PIN | LCD_SCLK_PIN
                                  | LCD_BLA_PIN | LCD_RST_PIN;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_Out_PP;
    GPIO_Init(LCD_CS_PORT, &GPIO_InitStructure);  /* all on GPIOE */
...
void LCD12864_SetPos(uint8_t row, uint8_t col)
{
    ...
    if (row > 3) row = 3;
    if (col > 7) col = 7;   /* 8 half-width chars per line in graphics mode */
