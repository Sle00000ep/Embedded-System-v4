...
/**
 * Write all 8 LEDs (writes ODR low byte, 0=on 1=off)
 * val: bit0=LED1 ... bit7=LED8
 */
void LED_WriteAll(uint8_t val)
{
    uint16_t odr = GPIO_ReadOutputData(LED_ALL_PORT);
    odr &= 0xFF00;              /* preserve PF8-PF15 state */
    odr |= (~val) & 0x00FF;    /* low 8 bits inverted (active-low) */
    GPIO_Write(LED_ALL_PORT, odr);
}
...
void LED_Set(uint8_t index, uint8_t on)
{
    if (index > 7) return;
    if (on)
        GPIO_ResetBits(LED_ALL_PORT, (uint16_t)(1 << index));  /* low = on */
    else
        GPIO_SetBits(LED_ALL_PORT, (uint16_t)(1 << index));    /* high = off */
}
